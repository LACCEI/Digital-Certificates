...
describe("DigitalCertificatesAPI", () => {
  ...
  // let digitalCertificatesManagerMock: jest.Mocked<DigitalCertificatesManager>;

  beforeEach(() => {
    // digitalCertificatesManagerMock = new DigitalCertificatesManager() as jest.Mocked<DigitalCertificatesManager>;
    // digitalCertificatesAPI = new DigitalCertificatesAPI();
    (DigitalCertificatesManager as jest.Mock<DigitalCertificatesManager>).mockClear();
    digitalCertificatesAPI = new DigitalCertificatesAPI();
  });
  ...
  describe("load CSV files", () => {
    ...
    it("should load data correctly from CSV file", async () => {
      const simple_csv_file_path = path.resolve(__dirname, "./sample-files/simple-list.csv");
      ...
      const template_filepath = path.resolve(__dirname, "./templates/test-template1.docx");
      ...
      const generate_certificatesMock = (DigitalCertificatesManager as jest.Mock<DigitalCertificatesManager>).mock.instances[0].generate_certificates;
